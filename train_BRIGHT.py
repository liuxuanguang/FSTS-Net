...
#from models.HGINet import HGINet as Net
# from models.Multimodal_SCD import mmscd_siam_GCN_concat as Net
# from models.WTHDC import mmscd_siam_GCN_WT as Net
...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        self.deep_model = Net("resnet34", False, 4, "lightweight", 6, 0.00005)
        # self.deep_model = Net(64, 4)

        # 加载预训练权重
        pretrained_path = '/mnt/nas/checkpoints/BRIGHT/SGF-Net_20260610_112649/best_model_val_68.2_test_67.1.pth'
        ...
        # 在初始化 criterion_bn 时（例如在 __init__ 方法中）
        self.criterion_bn = nn.BCEWithLogitsLoss(reduction='none')  # 替换原有的 BCELoss
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            # print('bcd', labels_loc.max())
            # print('scd', labels_clf.max())
            pre_change_imgs = pre_change_imgs.cuda()
            post_change_imgs = post_change_imgs.cuda()
            labels_loc = labels_loc.cuda().float()
            labels_clf = labels_clf.cuda().long()
            # use it for HGINet
            # labels_clf = labels_clf.cuda().float()
            valid_labels_clf = (labels_clf != 255).any()
            if not valid_labels_clf:
                continue

            out_scd, out_bcd = self.deep_model(pre_change_imgs, post_change_imgs)  # if you use UNet
            ...
            # 让 out_bcd 和 labels_loc 都去掉单通道维度（形状均为 [4,512,512]）
            #out_bcd = out_bcd.squeeze(1)  # 从 [4,1,512,512] → [4,512,512]
            #labels_loc = labels_loc.squeeze(1)  # 确保无通道维度
            # 再计算损失
    ...
    def validation(self):
        ...
        val_dataset = MultimodalDamageAssessmentDatset(self.args.val_dataset_path, self.args.val_data_name_list, 1024,
                                                       None, 'test')
        ...
        with torch.no_grad():
            for _, data in enumerate(val_data_loader):
                ...
                output_clf, output_loc = self.deep_model(pre_change_imgs, post_change_imgs)  # if you use UNet
                ...
                output_loc = torch.sigmoid(output_loc).detach().cpu().numpy()
...
def main():
    ...
    parser.add_argument('--train_dataset_path', type=str, default='/*****/BRIGHT')
    parser.add_argument('--train_data_list_path', type=str, default='/*****/BRIGHT/train_set.txt')
    parser.add_argument('--val_dataset_path', type=str, default='/*****/BRIGHT')
    parser.add_argument('--val_data_list_path', type=str, default='/*****/BRIGHT/val_set.txt')
    parser.add_argument('--test_dataset_path', type=str, default='/*****/BRIGHT')
    parser.add_argument('--test_data_list_path', type=str, default='/*****/test_set.txt')
    ...
    parser.add_argument('--model_type', type=str, default='SGF-Net')
