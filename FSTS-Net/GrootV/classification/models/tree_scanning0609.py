...
from GrootV.classification.models.tree_scan_utils.tree_scan_core0609 import MinimumSpanningTree, MinimumSpanning3DTree, MinimumSpanningMT3DTree, MinimumSpanningMTnDTree
...
def tree_MTscanning3D_core(xs, dts, As, Bs, Cs, Ds, delta_bias, origin_shape, h_norm):
    ...
    mst_layer = MinimumSpanningMT3DTree("Cosine", torch.exp)  # 创建最小生成树层，3D扫描代码
...
def tree_scanningMT3D_core(xs, dts, As, Bs, Cs, Ds, delta_bias, origin_shape, h_norm, Tem):
    ...
    fea4tree_hw = rearrange(xs, 'b d (h w) -> b d h w', h=H, w=W)  # B d L
    # mst_layer = MinimumSpanningTree("Cosine", torch.exp)  # 创建最小生成树层,原始代码
    mst_layer = MinimumSpanningMTnDTree("Cosine", torch.exp, Tem)  # 创建最小生成树层，3D扫描代码
    tree = mst_layer(fea4tree_hw)  # 根据特征构建最小生成树
...
def tree_scanning3D(
    ...
    ys = tree_scanningMTnD_core(xs, dts,
                            As, Bs, Cs, Ds,
