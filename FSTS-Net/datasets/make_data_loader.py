...
class MultimodalDamageAssessmentDatset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.dataset_path, 'target', self.data_list[index] + self.suffix)
...
'''
loading new dataset HDC dataset by this dataloader
'''
...
class NewDataset(Dataset):
    ...
    def __transforms(self, aug, pre_img, post_img, label):
        ...
        pre_img = imutils.normalize_img_newdata(pre_img)  # imagenet normalization
        pre_img = np.transpose(pre_img, (2, 0, 1))

        post_img = imutils.normalize_4band_img(post_img)  # imagenet normalization
        post_img = np.transpose(post_img, (2, 0, 1))
    ...
    def __getitem__(self, index):
        ...
        # 获取post-event图像路径 - 需要将PS-RGB替换为SAR-Intensity
        post_filename = self.data_list[index].replace('PS-RGB', 'SAR-Intensity')
        post_path = os.path.join(self.dataset_path, 'post-event', post_filename)
        label_path = os.path.join(self.dataset_path, 'target', self.data_list[index])
        # print(pre_path)
        # print(post_path)
        pre_img = self.loader(pre_path)[:, :, 0:3]
...
class NewDataset_Inference(Dataset):
    def __init__(self, dataset_path, data_list, data_loader=img_loader, suffix='.tif'):
        self.dataset_path = dataset_path
        self.data_list = data_list
        self.loader = data_loader
