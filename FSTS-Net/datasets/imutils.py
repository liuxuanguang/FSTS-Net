...
# new dataset
def normalize_img_newdata(img, mean=[98.48023508, 102.16419801, 85.95128999], std=[73.94596124, 59.27847929, 51.72256644]):
    """Normalize image by subtracting mean and dividing by std."""
    img_array = np.asarray(img)
    normalized_img = np.empty_like(img_array, np.float32)
...
def normalize_4band_img(img, mean=[19.25729969, 24.23937644, 22.72455696], std=[15.16593605, 18.27532921, 17.30868976]):
    """Normalize image by subtracting mean and dividing by std."""
    img_array = np.asarray(img)
    normalized_img = np.empty_like(img_array, np.float32)
...
# # wrrited by BRIGHT dataset
# def random_crop(pre_img, post_img, label, crop_size, mean_rgb=[0, 0, 0], ignore_index=0):
#     h, w = label.shape
#
#     H = max(crop_size, h)
#     W = max(crop_size, w)
...
# wrrited by new dataset
def random_crop(pre_img, post_img, label, crop_size, mean_rgb=[0, 0, 0, 0], ignore_index=0):
    h, w = label.shape
