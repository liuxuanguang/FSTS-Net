...
class CIEM(nn.Module):
    def __init__(self, in_d, out_d, Lambda):
        ...
        # 添加调试信息
        print(f"CIEM initialized with in_d={in_d}, out_d={out_d}")
        # 修改conv_dr的输入通道数为in_d*2（因为要拼接两个特征图）
        self.conv_dr = nn.Sequential(
            nn.Conv2d(in_d * 2, out_d, kernel_size=1, bias=True),  # 修改为in_d * 2
            nn.BatchNorm2d(out_d),
            nn.ReLU(inplace=True)
        )

        self.conv_cat = nn.Sequential(
            nn.Conv2d(in_d * 2, out_d, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(out_d),
            nn.ReLU(inplace=True)
        )
        ...
        self.bn2 = nn.BatchNorm2d(in_d)
        self.relu2 = nn.ReLU(inplace=True)
        self.bn3 = nn.BatchNorm2d(out_d)
        self.relu3 = nn.ReLU(inplace=True)
        self.SSFC1 = SimAM(e_lambda=self.Lambda)
        self.SSFC2 = SimAM(e_lambda=self.Lambda)

        # 添加一个卷积层来调整diff的通道数
        self.diff_adjust = nn.Sequential(
            nn.Conv2d(in_d, out_d, kernel_size=1),
            nn.BatchNorm2d(out_d),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, input1, input2):
        ...
        # 调整diff的通道数
        diff = self.diff_adjust(diff)

        feature1 = self.relu1(self.bn1(input1 * att + input1))
        feature2 = self.relu2(self.bn2(input2 * att + input2))
        ...
        different = self.relu3(self.bn3(feature_add))
        return different